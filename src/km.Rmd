...
kmfunc <- function(time, event, eventname, yposplus = rep(0, 6), lrxy = c(365 * .5, 0.95)) {
  ...
  text(lrxy[1], lrxy[2], paste0("Log-rank p ", p), pos = 4)
}
```
...
kmfunc(
  ...
  yposplus = c(0.0, -0.01, -0.01, -0, +0, 0.01) # case ref, control ref, case mref, control mref, case peg, control pef
)
```
...
kmfunc(
  ...
  # yposplus = c(-0.0, 0, 0, 0) # case ref, control ref, case mref, control mref, case peg, control pef
)
```
...
kmfunc(
  ...
  yposplus = c(0.052, -0.041, 0.026, -0.021, 0, 0), # case ref, control ref, case mref, control mref, case peg, control pef
  lrxy = c(365 * 2.1, 0.93)
)
```

```{r kmacdeath, fig.cap="1-KM All-cause mortality", cache=cacheon, dependson="km"}
kmfunc(
  time = "sos_outtime_death",
  event = "sos_out_death",
  eventname = "All-cause mortality (%)",
  yposplus = c(-0.005, 0.0, 0.005, -0.0, 0, 0) # case ref, control ref, case mref, control mref, case peg, control pef
)
```

```{r kmcvdeath, fig.cap="1-KM CV mortality", cache=cacheon, dependson="km"}
kmfunc(
  time = "sos_outtime_death",
  event = "sos_out_deathcv",
  eventname = "CV mortality (%)",
  yposplus = c(0.008, -0.008, -0.008, 0.008, 0, 0) # case ref, control ref, case mref, control mref, case peg, control pef
)
```

```{r kmnoncvdeath, fig.cap="1-KM Non-CV mortality", cache=cacheon, dependson="km"}
kmfunc(
  time = "sos_outtime_death",
  event = "sos_out_deathnoncv",
  eventname = "Non-CV mortality (%)",
  yposplus = c(-0.01, 0.0, 0.0, 0.0, 0, 0.01) # case ref, control ref, case mref, control mref, case peg, control pef
)
```
